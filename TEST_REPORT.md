...
**Tools Tested:**
- `db_list_databases()` → ✓ Working
- `db_list_tables()` → ✓ Working  
- `db_get_schema()` → ✓ Working
- `db_execute_query()` → ✓ Working (readonly mode enforced)
...
1. `db_list_databases()` - List all databases
2. `db_list_tables()` - List tables in schema
3. `db_get_schema()` - Get full table schema with columns/keys/FKs
4. `db_execute_query()` - Execute SELECT or DML queries
